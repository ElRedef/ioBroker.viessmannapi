...
async function extractKeys_features(adapter, extractPath, element, preferedArrayName, forceIndex, write, channelName) {
    try {
        ...
        for (const channel of channelNames) {
            addChannel(adapter, path+channel);
        }

        //Create Devices 
        adapter.log.debug("Creating Devices")
        for (const device of deviceNames) {
            addDevice(adapter, path+device);
        }

        //Create Commands 
        adapter.log.debug("Creating Commands")
        for (const command of commandNames) {
            addCommand(adapter,path+command.name, command.uri, command.params);
        }
        ...
        for (const datapoint of datapointNames) {
            ...
            addDataPoint(adapter, path+name, datapoint.prop, datapoint.feature, datapoint.uri);
        }
    } catch (error) {
