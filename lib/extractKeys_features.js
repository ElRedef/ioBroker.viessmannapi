 ...
 * 
 */
async function extractKeys_features(adapter, extractPath, element, preferedArrayName, forceIndex, write, channelName) {
    try {
        ...
        path = getPath(extractPath)
        adapter.log.debug("Extracting: "+extractPath+" to "+ path)

        //clear Arrays from last run
        deviceNames.splice(0, deviceNames.length)
        channelNames.splice(0, channelNames.length)
        commandNames.splice(0, commandNames.length)
        datapointNames.splice(0, datapointNames.length)
        ...
        for (const dataPoint of element.data) {
            await AnalyzeDatapoint(adapter,path, dataPoint);
        }

        adapter.log.debug("Gefundene Devices: " + deviceNames);
        adapter.log.debug("Gefundene Channels: " + channelNames);
        ...
        adapter.log.error("Error extract keys: " + extractPath + " " + JSON.stringify(element));
...
function getPath(extractPath)
{
    const words = extractPath.split('.');
 ...
 * 
*/
async function AnalyzeDatapoint(adapter,path,dataPoint)
{
    try{
        if(Object.keys(dataPoint.properties).length > 0) {//Nur anlegen wenn ein value vorhanden ist
            ...
            //filter unwanted keys (Units are also inside the specific value)
            keys = removeKey(keys,"unit");
            keys = removeKey(keys,"maxUnit");
            keys = removeKey(keys,"minUnit");
            ...
            for(const key in keys){
                ...
                let dp = {
                    "name": path+ name+ "."+ keys[key],
                    "uri": dataPoint.uri,
                    "prop": prop,
                    "feature": dataPoint.feature
                }

                if(keys.length==1) dp.name = path+ name; //if there is only one param; skip one level 
...
/*********************************************************************************************
 * Removes an 'filter'-value from an array
 */
function removeKey(array,filter) 
{
...
async function addCommand(adapter, cmd_name, cmd_uri, cmd_params) {
    ...
    if(keys.length > 0) 
    {
        type = params[keys[0]].type
    }

    if(type == 'Schedule') type = 'json'
