...
async function AnalyzeDatapoint(adapter,dataPoint)
{
    try{
        if(Object.keys(dataPoint.properties).length > 0) {//Nur anlegen wenn ein value vorhanden ist
            ...
            if(datapointNames.indexOf(datapoint) === -1)  datapointNames.push(datapoint);        
...
async function AddDataPoint(adapter, name, prop,  feature, uri) {
    ...
    //First decide which value (and type) we shall write:
    //  1.) If viessmann provides a value we use this
    //  2.) If there is no value attribute look for status or active information
    if (prop.value) { 
        ...
        value = prop.active.value;
        typ = prop.active.type;

    }
    else if (prop.status) 
    {
        value = prop.status.value;
        typ = prop.status.type;
    }
    else { //if it isn' one of those write a JSON 
        value = JSON.stringify(prop);
        typ = 'json';
    }

    //Sometimes a value is present and this value has an status
    if (prop.status) 
    {
