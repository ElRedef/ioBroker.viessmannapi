...
async function extractKeys(adapter, path, element, preferedArrayName, forceIndex, write, channelName) {
    try {
        ...
        //adapter.log.error("Path:" + path + " "); //JF
        //adapter.log.error("element: " + JSON.stringify(element) ); //JF
        //adapter.log.error("keys: " + objectKeys ); //JF
        //adapter.log.error("preferedArrayName:" + preferedArrayName + " "); //JF
        //adapter.log.error("channelName:" + channelName + " "); //JF
        ...
        //element.data.forEach(dataPoint => {adapter.log.error("Found: " + dataPoint.feature)});
...
async function addDataPoint(adapter,dataPoint)
{
    try{
        ...
        if(Object.keys(dataPoint.properties).length > 0) {//Nur anlegen wenn eine value vorhanden ist
            ...
            adapter.log.info("Adding: " + dataPoint.feature)
        ...
        {
            adapter.log.info("Skipping: " + dataPoint.feature)
        }
